...
![Password Denied](h2_c3_password_denied.png)
...
Korjasin tämän antamalla kyseisiin kansioihin omistajaryhmälle ja muille käyttäjille luku ja execute oikeudet (`chmod 755`) ja tarkistin forbidden virheen kadonneen. Tämä oli lähtötilanne tähän tehtävään.
...
- 6 - 110 - rw-
- 7 - 111 - rwx
Käyttäjät määritellään järjestyksessä omistaja - omistajaryhmä - muut, joten `chmod 750` antaisi omistajalle kaikki oikeudet (7 -> 111 -> rwx), omistajaryhmälle (5 -> 101 -> r-x) luku ja execute oikeudet, ja muille (0 -> 000 -> ---) ei mitään oikeuksia.

- http://jimmonen.me/~janne/
Koska minulla oli jo käyttäjien kotisivut aktivoituina (`sudo a2enmod userdir`) päätin käyttää sitä ensimmäiseen sivustoon. Varmistin ensin yhteydenoton onnistuvan.
...
![VH edited](h5_a7_vh_edited.png)

Korjasin vielä puuttuvan .conf lopun `alisivu.jimmonen.me` nimessä ja käynnistin hostit, jonka jälkeen uudelleenkäynnistin apachen.
...
![Wildcard Works](h5_b4_wildcard_workds.png)
...
![Wildcard Browser](h5_b6_wildard_browser.png)
...
Aloitin tehtävän katsomalla [1Tony Teaches Tech youtube videon](https://www.youtube.com/watch?v=iESSCDnC74k) Digin peruskomennoista ja niiden lukemisesta.
...
Ajoin sen jälkeen parametrittömät host ja dig kyselyt osoitteille jimmonen.me, alisivu.jimmonen.me ja wildcard.jimmonen.me osoitteille.

Tulokset Namecheapin asetusten mukaisesti; jimmonen.me ja alisivu.jimmonen.me viittasivat samaan IP-osoitteeseen ja wildcard oli alias jimmonen.me sivulle.

![Host jimmonen](h5_d2_host_jimmonen.png)

![Dig jimmonen](h5_d3_dig_jimmonen.png)

Digin tulosteessa `ANSWER SECTION:` viittaa vastaukseen.
`jimmonen.me.	11	IN	A	104.248.140.126` olisi aukikirjoitettuna
`jimmonen.me.` - Domain nimi `11` TTL, 11 sekuntia jäljellä(300 jos kyselyhetkellä se on vanhentunut) `IN` Internet `A` A Record `104.248.140.126` IP-osoite.
- poe.re
